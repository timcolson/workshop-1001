...
Open `templates/recipe_list_fragment.html` and find the recipe detail link:
...
Open `templates/recipe_list_fragment.html` and add another attribute to the detail link:

```html title="templates/recipe_list_fragment.html" ins={4-5}
<a href="#"
   hx-get="{{ url_for('recipe_detail', recipe_id=recipe.index) }}"
   hx-target="#recipe-details"
