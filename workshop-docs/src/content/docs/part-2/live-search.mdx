...
def search_recipes():
    ...
    return render_template('search_results_fragment.html',
...
For the search results only, create a new template file: `templates/search_results_fragment.html`

```html showLineNumbers title="templates/search_results_fragment.html" 
{% if query %}
<p class="search-info">
